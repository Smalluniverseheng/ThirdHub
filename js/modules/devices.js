...
export async function registerDevice() {
  ...
  await syncPush('th_user_devices', { id, data: info }, u.id);

  /* 超过 20 台 → 清除最久未使用的（保留本机） */
  try {
    const rows = await syncPull('th_user_devices', u.id);
    if (rows.length > MAX_DEVICES) {
      ...
      for (const r of sorted.slice(0, excess)) {
        await sb.from('th_user_devices').delete().eq('id', r.id);
      }
    }
  } catch (e) {}
}
      ...
      async function renderList() {
        ...
        rows.forEach((r) => {
          ...
          if (rm) rm.onclick = async () => {
            ...
            await getSupabase().from('th_user_devices').delete().eq('id', r.id);
            toast('已移除', 'ok');
            renderList();
          };
