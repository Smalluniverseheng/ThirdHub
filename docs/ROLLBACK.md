...
| v2.8 | 待补推（同 v2.7，等新 token） | 待补 | `f860b407`（代码）、`8a7a3042`（含回滚文档） | `e7d2529` |
| v2.9 | 待补推（同 v2.7，等新 token） | 待补 | `5e0a4886`（旧连接器自动升级新规则引擎） | `d87c1b6` |
| v3.0 | 待补推（同 v2.7，等新 token） | 待补 | `7f2d6269`（规则引擎 id./class./tag./text./!N/## + 源仓库 + 裸URL导入） | `e9bd0f9` |
| v3.1 | 待补推（同 v2.7，等新 token） | 待补 | `80025fa9`（搜索列表左图右文、封面/插图中转、云端组件本地化） | `239d940` |
| v3.2 | 待补推（同 v2.7，等新 token） | 待补 | `8ab9abda`（板块默认书架、搜索收起放大镜） | `b4c435c` |
| v3.3 | 待补推（同 v2.7，等新 token） | 待补 | `7c1e6a07`（正文HTML渲染修复、听书控制条+语速+连读） | `f89785a` |
| v3.4 | 待补推（同 v2.7，等新 token） | 待补 | `abd3d8cc`（下拉添加书签、目录面板书签页签） | `a03bbae` |
| v3.5 | 待补推（同 v2.7，等新 token） | 待补 | `d81ac029`（书源正文修复 @css:/tocUrl/replaceRegex/POST+GBK、阅读器底栏仿番茄、公告刷新、板块纯书架） | `15b505f` |
| v3.6 | 待补推（同 v2.7，等新 token） | 待补 | `aa25396c`（Venera 运行时补 loadSetting、连接器筛选+一键全测+动画、设置控件全局化、AI 图标库+模型更新 2026-08） | `40587ff` |
| v3.7 | 待补推（同 v2.7，等新 token） | 待补 | `d3810180`（代理黑名单转发修复漫画图片、同步 hmac、小说/漫画/有声合并「阅读」板块、Lucide 设置齿轮） | `7f0bdca` |
| v3.8 | 待补推（同 v2.7，等新 token） | 待补 | `8e605848`（类型筛选联动连接器列表、源内单源搜索、有声类型自动识别、推荐仓库、TVbox 仓库/视频解析增强） | `dd9da0f` |
| v3.9 | 待补推（同 v2.7，等新 token） | 待补 | `88a00a34`（Venera 引擎对齐官方：搜索选项默认值、loadNext、异步 init、IIFE 隔离、同步 AES/MD5/SHA/HMAC、APP 全局、JSON Content-Type） | `e22e130` |
