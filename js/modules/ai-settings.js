...
/* ---------- v6.6 中转密钥：第三方中转站通用密钥（多品牌） ---------- */
function subRelay() {
  openOverlay({
    title: '中转密钥',
    build: async (body) => {
      body.innerHTML = '<div class="set-wrap col gap8" id="relay-root"></div>';
      const root = $('#relay-root', body);
      const rel = await import('../ai/ai-api.js');
      const render = async () => {
        const keys = await rel.getRelayKeys();
        root.innerHTML = '<div class="muted" style="font-size:12.5px;line-height:1.8">第三方中转站密钥可同时调用多个品牌模型（OpenAI / DeepSeek / 智谱等）。保存后自动匹配到所有支持厂商，切换模型照常工作，无需逐个配置。</div>' +
          '<div class="row gap8" style="margin-top:10px"><input class="input" data-f="key" type="password" placeholder="粘贴中转站 API Key（sk-...）" style="flex:1"><button class="btn btn-primary" data-a="probe">检测并保存</button></div>' +
          '<div data-v="result"></div>' +
          '<div class="section-title" style="margin-top:12px">已保存的中转密钥</div><div data-v="list"></div>';
        const resBox = $('[data-v="result"]', root);
        const listBox = $('[data-v="list"]', root);
        listBox.innerHTML = keys.length ? keys.map(function (k, i) {
          return '<div class="row gap8" style="padding:8px 0;border-bottom:1px solid var(--line)">' +
            '<div class="grow" style="min-width:0"><div style="font-size:13.5px;font-weight:600">' + esc(k.name || '中转站模型') + '</div><div class="muted" style="font-size:11.5px">' + String(k.key || '').slice(0, 8) + '… · ' + esc(k.note || '') + '</div></div>' +
            '<button class="btn btn-sm" data-rn="' + i + '">重命名</button>' +
            '<button class="btn btn-sm btn-danger" data-rd="' + i + '">删除</button></div>';
        }).join('\n') : '<div class="muted" style="font-size:12.5px">还没有中转密钥</div>';
        $$('[data-rd]', listBox).forEach(function (b) { b.onclick = async function () { await rel.removeRelayKey(keys[+b.dataset.rd].id); toast('已删除', 'ok'); render(); }; });
        $$('[data-rn]', listBox).forEach(function (b) { b.onclick = async function () {
          var v = prompt('密钥名称（如：我的中转站）', keys[+b.dataset.rn].name || '中转站模型');
          if (v) { keys[+b.dataset.rn].name = v.trim(); await kvSet('ai:relay-keys', keys); toast('已重命名', 'ok'); render(); }
        }; });
        $('[data-a="probe"]', root).onclick = async function () {
          var key = ($('[data-f="key"]', root).value || '').trim();
          if (!key) return toast('请先粘贴密钥', 'err');
          var btn = $('[data-a="probe"]', root); btn.disabled = true; btn.textContent = '检测中（并行探测 3 家品牌）…';
          var r = await rel.probeRelayKey(key);
          btn.disabled = false; btn.textContent = '检测并保存';
          if (r.relay) {
            var s = await rel.saveRelayKey(key);
            resBox.innerHTML = '<div class="tag" style="background:#22c55e22;color:#22c55e;padding:6px 10px;margin-top:8px;display:inline-block">✅ 检测到中转站密钥：' + r.ok + '/3 家品牌可用，已自动匹配到 ' + s.count + ' 个支持厂商</div>';
            toast('中转密钥已保存并自动匹配全部支持厂商', 'ok');
            render();
          } else {
            resBox.innerHTML = '<div class="muted" style="margin-top:8px;font-size:12.5px">检测结果：' + r.results.map(function (x) { return x.pid + (x.ok ? ' ✓' : ' ✗'); }).join(' · ') + ' —— 可用品牌不足 2 家，可能不是中转站密钥（普通密钥请在对应厂商处配置）</div>';
          }
        };
      };
      render();
