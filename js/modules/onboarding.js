...
import { PROVIDERS, modelIdOf, modelNickOf } from '../ai/ai-models.js';
...
const FAQS = [
  ...
  { q: '我的 API Key 安全吗？', a: 'Key 只保存在你设备的本地数据库中。默认所有请求直接从你的浏览器发往厂商接口；仅当你主动开启「AI 后端」（云端会员后端或自建本地后端）时，Key 才会随对话请求转发给你所选的后端用于调用厂商，后端不保存 Key。' },
...
export async function maybeOnboard() {
  ...
  return new Promise((resolve) => {
    ...
    function stepLanding() {
      ...
      // 跑马灯内容（模型名，双份无缝循环）
      const marqueeModels = [];
      PROVIDERS.forEach((p) => (p.models || []).slice(0, 2).forEach((m) => marqueeModels.push(modelNickOf(m))));
      const marquee = marqueeModels.map((m) => `<span class="obl-mq-chip">${esc(String(m))}</span>`).join('');
            ...
            <div class="obl-lib">
              ${libVendors.map((p) => `
                <div class="obl-lib-row">
                  ...
                  <div class="obl-lib-models">${[...(p.models || []), ...(p.image || []), ...(p.video || [])].slice(0, 6).map((m) => `<span class="obl-chip sm">${esc(typeof m === 'string' ? m : modelNickOf(m))}</span>`).join('')}</div>
                </div>`).join('')}
            </div>
