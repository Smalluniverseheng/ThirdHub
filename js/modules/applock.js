/* ===== ThirdHub js/modules/applock.js — 应用锁（v2.0） =====
   三种解锁方式：6 位数字密码 / 字母数字混合密码 / 九宫格图案
   开启后每次进入应用需先解锁；本机存储哈希，不上传云端
   v2.0：新增字母数字混合密码；数字键盘支持电脑物理按键输入；忘记密码可通过登录密码或邮箱验证码找回 */
import { $, $$, el, esc, icon, toast, openOverlay, modal, confirmDialog } from '../ui.js';
import { kvGet, kvSet, kvDel } from '../store.js';

async function hash(s) {
  const buf = await crypto.subtle.digest('SHA-256', new TextEncoder().encode('th-lock:' + s));
  return [...new Uint8Array(buf)].map((b) => b.toString(16).padStart(2, '0')).join('');
}

export async function lockConfig() {
  return { type: await kvGet('applock:type', null), hash: await kvGet('applock:hash', '') };
}

export async function verifyLock(secret) {
  const c = await lockConfig();
  if (!c.type) return true;
  return (await hash(secret)) === c.hash;
}

const TYPE_HINT = {
  pin: '请输入 6 位数字密码',
  mix: '请输入字母数字密码',
  pattern: '请绘制解锁图案',
};
const TYPE_NAME = { pin: '数字密码', mix: '字母数字密码', pattern: '图案密码' };

/* ================= 解锁界面（启动门禁） ================= */
export async function gateIfLocked() {
  const c = await lockConfig();
  if (!c.type) return;
  await new Promise((resolve) => {
    const mask = el(`<div class="applock-mask">
      <div class="applock-box">
        <div class="applock-logo">${icon('lock')}</div>
        <div style="font-size:17px;font-weight:800;margin-bottom:4px">ThirdHub 已锁定</div>
        <div class="muted mb16" id="lk-hint">${TYPE_HINT[c.type]}</div>
        <div id="lk-body"></div>
        <button class="lk-forgot" data-a="forgot">忘记密码？</button>
      </div>
    </div>`);
    document.body.appendChild(mask);
    const done = () => { mask.remove(); resolve(); };
    const fail = () => toast('密码不正确，请重试', 'err');
    const body = $('#lk-body', mask);
    const tryUnlock = async (v) => { if (await verifyLock(v)) done(); else fail(); };
    renderPad(body, c.type, tryUnlock);
    /* 忘记密码：登录密码或邮箱验证码验证身份后解除本机锁 */
    $('[data-a="forgot"]', mask).onclick = () => showForgotLock(body, mask, c.type, tryUnlock, done);
  });
}

/* 渲染对应类型的解锁面板 */
function renderPad(body, type, tryUnlock) {
  body.innerHTML = '';
  if (type === 'pin') buildPinPad(body, tryUnlock);
  else if (type === 'mix') buildMixPad(body, tryUnlock);
  else buildPatternPad(body, tryUnlock);
}

/* ================= 忘记密码（登录密码 / 邮箱验证码） ================= */
async function showForgotLock(body, mask, curType, tryUnlock, done) {
  const { currentUser } = await import('../auth.js');
  const u = await currentUser().catch(() => null);
  const email0 = (u && u.email) || '';
  body.innerHTML = `
    <div class="lk-recover">
      <div class="muted" style="font-size:12.5px;line-height:1.8;margin-bottom:12px">忘记应用锁密码？验证账号身份后可解除本机应用锁（解除后需重新设置）。</div>
      <div class="row gap8" style="margin-bottom:10px">
        <button class="btn grow sel" data-a="tab-pwd">登录密码</button>
        <button class="btn grow" data-a="tab-code">邮箱验证码</button>
      </div>
      <div id="fl-form"></div>
      <button class="lk-back" data-a="back" style="margin-top:12px">← 返回解锁</button>
    </div>`;
  const form = $('#fl-form', body);
  const clearLock = async () => {
    await kvDel('applock:type'); await kvDel('applock:hash');
    toast('应用锁已解除', 'ok');
    done();
  };
  const renderForm = (tab) => {
    $$('[data-a^="tab-"]', body).forEach((b) => b.classList.toggle('sel', b.dataset.a === 'tab-' + tab));
    if (tab === 'pwd') {
      form.innerHTML = `
        <div class="col gap8">
          <input class="input" type="email" placeholder="登录邮箱" value="${esc(email0)}" data-f="email">
          <input class="input" type="password" placeholder="登录密码" data-f="pwd">
          <button class="btn btn-primary" data-a="go">验证并解除锁</button>
        </div>`;
      $('[data-a="go"]', form).onclick = async () => {
        const { signIn } = await import('../auth.js');
        const btn = $('[data-a="go"]', form); btn.disabled = true;
        try {
          await signIn($('[data-f="email"]', form).value.trim(), $('[data-f="pwd"]', form).value);
          await clearLock();
        } catch (e) { btn.disabled = false; toast(e.message || '验证失败', 'err'); }
      };
    } else {
      form.innerHTML = `
        <div class="col gap8">
          <input class="input" type="email" placeholder="登录邮箱" value="${esc(email0)}" data-f="email">
          <div class="row gap8">
            <input class="input grow" placeholder="6 位验证码" maxlength="6" inputmode="numeric" data-f="code">
            <button class="btn" data-a="send" style="flex:none;min-width:88px">获取验证码</button>
          </div>
          <button class="btn btn-primary" data-a="go">验证并解除锁</button>
        </div>`;
      const sendBtn = $('[data-a="send"]', form);
      sendBtn.onclick = async () => {
        const { sendEmailCode } = await import('../email-code.js');
        const em = $('[data-f="email"]', form).value.trim();
        if (!/^[^@\s]+@[^@\s]+\.[^@\s]+$/.test(em)) return toast('请先填写正确的邮箱', 'err');
        sendBtn.disabled = true; sendBtn.textContent = '发送中…';
        try {
          await sendEmailCode(em);
          toast('验证码已发送', 'ok');
          let n = 59;
          const t = setInterval(() => {
            sendBtn.textContent = n > 0 ? `重新发送（${n}s）` : '重新发送';
            sendBtn.disabled = n > 0;
            if (n <= 0) clearInterval(t);
            n--;
          }, 1000);
        } catch (e) { sendBtn.disabled = false; sendBtn.textContent = '获取验证码'; toast(e.message || '发送失败', 'err'); }
      };
      $('[data-a="go"]', form).onclick = async () => {
        const { verifyEmailCode } = await import('../email-code.js');
        const btn = $('[data-a="go"]', form); btn.disabled = true;
        try {
          await verifyEmailCode($('[data-f="email"]', form).value.trim(), $('[data-f="code"]', form).value.trim());
          await clearLock();
        } catch (e) { btn.disabled = false; toast(e.message || '验证码不正确', 'err'); }
      };
    }
  };
  $$('[data-a^="tab-"]', body).forEach((b) => b.onclick = () => renderForm(b.dataset.a.slice(4)));
  $('[data-a="back"]', body).onclick = () => renderPad(body, curType, tryUnlock);
  renderForm('pwd');
}

/* ================= 6 位数字键盘（支持电脑物理键盘输入） ================= */
function buildPinPad(box, onDone) {
  let pin = '';
  box.innerHTML = `
    <div class="lk-dots" id="lk-dots">${'<span class="lk-dot"></span>'.repeat(6)}</div>
    <div class="lk-pad">
      ${[1, 2, 3, 4, 5, 6, 7, 8, 9, '', 0, '⌫'].map((k) => k === '' ? '<span></span>' : `<button class="lk-key" data-k="${k}">${k}</button>`).join('')}
    </div>`;
  const dots = $$('.lk-dot', box);
  const render = () => dots.forEach((d, i) => d.classList.toggle('on', i < pin.length));
  const press = (k) => {
    if (k === '⌫') pin = pin.slice(0, -1);
    else if (pin.length < 6) pin += k;
    render();
    if (pin.length === 6) { const v = pin; pin = ''; setTimeout(render, 200); onDone(v); }
  };
  $$('.lk-key', box).forEach((b) => b.onclick = () => press(b.dataset.k));
  /* v2.0：电脑键盘数字键/退格键直接输入 */
  const onKey = (e) => {
    if (!box.isConnected) { document.removeEventListener('keydown', onKey); return; }
    if (/^[0-9]$/.test(e.key)) { e.preventDefault(); press(e.key); }
    else if (e.key === 'Backspace') { e.preventDefault(); press('⌫'); }
  };
  document.addEventListener('keydown', onKey);
}

/* ================= 字母数字混合密码（输入框 + 显示/隐藏 + 回车提交） ================= */
function buildMixPad(box, onDone, opts = {}) {
  const { placeholder = '输入密码', btnText = '解锁' } = opts;
  box.innerHTML = `
    <div class="lk-mix">
      <input class="input lk-mix-input" type="password" placeholder="${placeholder}" autocomplete="off" data-a="in">
      <button class="lk-mix-eye" data-a="eye" type="button">${icon('eye')}</button>
    </div>
    <button class="btn btn-primary lk-mix-go" data-a="go">${btnText}</button>`;
  const input = $('[data-a="in"]', box);
  const eye = $('[data-a="eye"]', box);
  const submit = () => {
    const v = input.value.trim();
    if (!v) return toast('请输入密码', 'err');
    input.value = '';
    onDone(v);
  };
  eye.onclick = () => {
    const show = input.type === 'password';
    input.type = show ? 'text' : 'password';
    eye.innerHTML = icon(show ? 'eyeOff' : 'eye');
  };
  $('[data-a="go"]', box).onclick = submit;
  input.addEventListener('keydown', (e) => { if (e.key === 'Enter') { e.preventDefault(); submit(); } });
  input.focus();
}

/* 九宫格图案板（触屏 + 鼠标通用） */
function buildPatternPad(box, onDone, min = 4) {
  box.innerHTML = `<div class="lk-pattern" id="lk-pat">
    ${Array.from({ length: 9 }, (_, i) => `<span class="lk-pdot" data-i="${i}"><i></i></span>`).join('')}
    <svg class="lk-lines" viewBox="0 0 240 240"><polyline id="lk-line" fill="none" stroke="var(--primary)" stroke-width="3" stroke-linecap="round"/></svg>
  </div>
  <div class="muted" style="text-align:center;margin-top:10px;font-size:12px">至少连接 ${min} 个点</div>`;
  const pad = $('#lk-pat', box);
  const line = $('#lk-line', box);
  const seq = [];
  let drawing = false;
  const center = (i) => {
    const d = $(`.lk-pdot[data-i="${i}"]`, pad);
    const r = d.getBoundingClientRect(), pr = pad.getBoundingClientRect();
    return [(r.left + r.width / 2 - pr.left) * 240 / pr.width, (r.top + r.height / 2 - pr.top) * 240 / pr.height];
  };
  const redraw = () => line.setAttribute('points', seq.map((i) => center(i).join(',')).join(' '));
  const hit = (x, y) => {
    const els = document.elementsFromPoint(x, y);
    const d = els.find((e2) => e2.classList && e2.classList.contains('lk-pdot'));
    return d ? +d.dataset.i : null;
  };
  const add = (i) => {
    if (i == null || seq.includes(i)) return;
    seq.push(i);
    $(`.lk-pdot[data-i="${i}"]`, pad).classList.add('on');
    redraw();
  };
  const pos = (e) => (e.touches && e.touches[0]) ? [e.touches[0].clientX, e.touches[0].clientY] : [e.clientX, e.clientY];
  const start = (e) => { drawing = true; seq.length = 0; $$('.lk-pdot', pad).forEach((d) => d.classList.remove('on')); redraw(); add(hit(...pos(e))); e.preventDefault(); };
  const move = (e) => { if (!drawing) return; add(hit(...pos(e))); e.preventDefault(); };
  const end = () => {
    if (!drawing) return;
    drawing = false;
    if (seq.length >= min) onDone(seq.join('-'));
    else { setTimeout(() => { seq.length = 0; $$('.lk-pdot', pad).forEach((d) => d.classList.remove('on')); redraw(); }, 300); }
  };
  pad.addEventListener('touchstart', start, { passive: false });
  pad.addEventListener('touchmove', move, { passive: false });
  pad.addEventListener('touchend', end);
  pad.addEventListener('mousedown', start);
  pad.addEventListener('mousemove', move);
  pad.addEventListener('mouseup', end);
  pad.addEventListener('mouseleave', end);
}

/* ================= 设置页（安全 → 应用锁） ================= */
export async function showAppLockSettings() {
  const c = await lockConfig();
  openOverlay({
    title: '应用锁',
    build: async (body) => {
      body.innerHTML = `<div class="set-wrap">
        <div class="muted" style="line-height:1.7;margin-bottom:14px">开启应用锁后，每次进入 ThirdHub 都需要先验证密码。密码仅保存在本机，忘记后可通过登录密码或邮箱验证码找回。</div>
        <div class="col gap8" id="lk-list"></div>
      </div>`;
      const list = $('#lk-list', body);
      const row = (name, desc, active, onClick) => {
        const b = el(`<button class="search-svc ${active ? 'sel' : ''}" style="width:100%">
          <span class="search-svc-radio">${active ? icon('check') : ''}</span>
          <div class="grow" style="text-align:left"><div style="font-size:14px;font-weight:600">${name}</div><div class="muted">${desc}</div></div>
        </button>`);
        b.onclick = onClick;
        list.appendChild(b);
      };
      row('关闭应用锁', '进入应用无需验证', !c.type, async () => {
        if (!c.type) return;
        await promptVerify(c, async () => {
          await kvDel('applock:type'); await kvDel('applock:hash');
          toast('应用锁已关闭', 'ok');
          showAppLockSettingsRefresh(body);
        });
      });
      row('数字密码', '6 位数字 PIN（支持电脑键盘输入）', c.type === 'pin', () => setupLock('pin', c, body));
      row('字母数字密码', '字母+数字混合，至少 6 位（推荐）', c.type === 'mix', () => setupLock('mix', c, body));
      row('图案密码', '九宫格图案（至少连接 4 个点）', c.type === 'pattern', () => setupLock('pattern', c, body));
    },
  });
}
function showAppLockSettingsRefresh(body) {
  /* 简单刷新：关闭重开 */
  const mask = body.closest('.overlay');
  if (mask) { mask.remove(); showAppLockSettings(); }
}

/* 已设置时先验证旧密码 */
async function promptVerify(c, onOk) {
  if (!c.type) return onOk();
  const m = el(`<div id="pv-body"></div>`);
  const ref = modal({ title: '验证当前密码', body: m });
  const ok = async () => { ref.close(); await onOk(); };
  if (c.type === 'pin') buildPinPad(m, async (v) => { if (await verifyLock(v)) ok(); else toast('密码不正确', 'err'); });
  else if (c.type === 'mix') buildMixPad(m, async (v) => { if (await verifyLock(v)) ok(); else toast('密码不正确', 'err'); }, { btnText: '验证' });
  else buildPatternPad(m, async (seq) => { if (await verifyLock(seq)) ok(); else toast('图案不正确', 'err'); });
}

/* 设置新密码（输入两遍确认） */
async function setupLock(type, c, body) {
  const doSetup = () => {
    openOverlay({
      title: type === 'pin' ? '设置数字密码' : type === 'mix' ? '设置字母数字密码' : '设置图案密码',
      build: (b2) => {
        b2.innerHTML = `<div class="set-wrap" style="display:flex;flex-direction:column;align-items:center">
          <div class="muted mb16" id="lk-step">第 1 步：输入新${type === 'pin' ? '密码' : type === 'mix' ? '密码（至少 6 位，字母+数字）' : '图案'}</div>
          <div id="lk-setup"></div>
        </div>`;
        let first = null;
        const handler = async (v) => {
          if (type === 'mix' && v.length < 6) { toast('密码至少 6 位', 'err'); return; }
          if (type === 'mix' && !/[A-Za-z]/.test(v)) { toast('密码需包含字母', 'err'); return; }
          if (type === 'mix' && !/\d/.test(v)) { toast('密码需包含数字', 'err'); return; }
          if (first == null) {
            first = v;
            $('#lk-step', b2).textContent = '第 2 步：再次输入以确认';
          } else if (v === first) {
            await kvSet('applock:type', type);
            await kvSet('applock:hash', await hash(v));
            toast('应用锁已开启', 'ok');
            const mask = b2.closest('.overlay');
            if (mask) mask.remove();
          } else {
            first = null;
            $('#lk-step', b2).textContent = '两次输入不一致，请重新设置';
            toast('两次输入不一致', 'err');
          }
        };
        if (type === 'pin') buildPinPad($('#lk-setup', b2), handler);
        else if (type === 'mix') buildMixPad($('#lk-setup', b2), handler, { placeholder: '字母+数字，至少 6 位', btnText: '确认' });
        else buildPatternPad($('#lk-setup', b2), handler);
      },
    });
  };
  if (c.type) await promptVerify(c, doSetup);
  else doSetup();
}
