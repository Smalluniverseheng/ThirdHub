...
async function loadDiscoverBox(box) {
  ...
  box.innerHTML = devs.map((d) => {
    ...
    return '<div class="cp-device" data-did="' + esc(d.device_id) + '" data-ip="' + esc(ip) + '">' +
      '<div class="cp-dev-head">' +
        '<span class="cp-dev-ico">' + icon('cpu') + '</span>' +
        '<div style="min-width:0">' +
          '<div class="cp-dev-name">' + esc(d.name || d.device_id) + '</div>' +
          '<div class="cp-dev-addr">' + esc(ip) + ':9600 · ' + (isMine ? '已绑定' : '未绑定') + '</div>' +
        '</div>' +
        '<span class="cp-dev-status"><span class="cp-dot on"></span>在线</span>' +
      '</div>' +
      '<div class="cp-actions">' +
        (isMine
          ? '<button class="btn btn-sm btn-primary" data-a="pconn">连接</button>'
  ...
  $$('[data-a="pair"]', box).forEach((b) => b.onclick = async () => {
    ...
    if (r.ok) {
      ...
      if (entry) { const c = await connectDevice(entry, { silent: true }); if (!c.ok) toast(c.error, 'err'); }
