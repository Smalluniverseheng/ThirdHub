/* ===== ThirdHub keyvault.js — 密钥保险库（二级密码 · 本地加密 · 云端托管） =====
   三种存储模式：
     local       仅保存本机 — 不上传云端
     cloud-enc   加密上传云端（默认）— 用二级密码 PBKDF2+AES-GCM 加密后上传，跨设备可用，更安全
     cloud-plain 明文上传云端 — 跨设备方便，未加密
...
export const KEY_MODES = [
  ...
  { id: 'cloud-enc', name: '加密上传云端', desc: '二级密码加密后上传，跨设备可用，更安全' },
  { id: 'cloud-plain', name: '明文上传云端', desc: '跨设备方便，未加密' },
];
...
export async function getKeyMode(providerId) {
  return await kvGet('ai:keymode:' + providerId, 'cloud-enc');
}
