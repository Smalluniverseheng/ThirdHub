...
function repoRenderList(body) {
  ...
  loadSb().then(function (cli) { return cli.rpc('admin_repo_list', { pwd: PWD }); })
    .then(function (r) {
      ...
      /* v6.4：两级树状分组 —— 健康源 / 18+ 源 → 分类 */
      var zones = { health: [], adult: [] };
      rows.forEach(function (x) { zones[x.zone === 'adult' ? 'adult' : 'health'].push(x); });
      function zoneUI(label, emoji, zrows, open) {
        if (!zrows.length) return '';
        var groups = {};
        zrows.forEach(function (x) { var c = x.category || '其他'; (groups[c] = groups[c] || []).push(x); });
        return '<details' + (open ? ' open' : '') + ' style="border:1px solid #2a2f3a;border-radius:10px;margin-bottom:10px;background:#12151c">' +
          '<summary style="cursor:pointer;padding:10px 12px;font-size:14px;font-weight:700;list-style:none;display:flex;align-items:center;gap:6px">' + emoji + ' ' + esc(label) + ' <span class="adm-muted" style="font-weight:400">（' + zrows.length + '）</span><span style="margin-left:auto;opacity:.6">▸</span></summary>' +
          '<div style="padding:0 10px 10px">' +
          Object.keys(groups).sort().map(function (c) {
            return '<div style="border-top:1px dashed #2a2f3a;margin-top:4px">' +
              '<div style="padding:8px 2px 4px;font-size:13px;font-weight:600"><span style="color:#9aa3b2">' + esc(c) + '</span> <span class="adm-muted">（' + groups[c].length + '）</span></div>' +
      ...
      box.innerHTML = zoneUI('健康源', '🟢', zones.health, true) + zoneUI('18+ 源', '🔞', zones.adult, false);
      $$('[data-act="repo-del"]', box).forEach(function (b) {
        b.onclick = function () {
          if (!confirm('从官方仓库删除这个源？用户端将无法再取用（已导入的不受影响）。')) return;
          loadSb().then(function (cli) { return cli.rpc('admin_repo_delete', { pwd: PWD, ids: [b.getAttribute('data-id')] }); })
            .then(function (rr) { if (rr.error) throw rr.error; toast('已删除'); repoRenderList(body); })
