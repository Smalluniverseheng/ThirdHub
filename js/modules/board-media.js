...
export async function renderMediaBoard(page, type) {
  ...
  page.innerHTML = `
    <div class="page-head"><div class="page-title">${NAME}</div>
      ...
      <button class="icon-btn" data-a="modset" title="设置">${icon('settings')}</button>
    </div>
  ...
  /* v7.0 模块独立设置：阅读/小说/漫画/音乐/有声/视频各自设置 + 连接器管理；「我的」为全局设置 */
  const modsetBtn = $('[data-a="modset"]', page);
  if (modsetBtn) modsetBtn.onclick = async () => {
    const ms = await import('./mod-settings.js');
    const { actionSheet, openOverlay } = await import('../ui.js');
    const items = [];
    if (type === 'novel' || isRead) items.push({ label: '小说阅读设置', value: 'novel', icon: 'book' });
    if (type === 'comic' || isRead) items.push({ label: '漫画阅读设置', value: 'comic', icon: 'comic' });
    if (type === 'music' || type === 'audio' || isRead) items.push({ label: '有声播放设置', value: 'audio', icon: 'headphone' });
    if (type === 'video') items.push({ label: '视频播放设置', value: 'video', icon: 'film' });
    items.push({ label: '连接器管理', value: 'connector', icon: 'plug' });
    const v = await actionSheet('设置', items);
    if (v === 'novel') return ms.showNovelSettings();
    if (v === 'comic') return ms.showComicSettings();
    if (v === 'audio' || v === 'video') { const m = await import('./mod-settings.js'); if (m.showPlayerSettings) m.showPlayerSettings(v); else toast('播放设置在后续版本开放', 'ok'); }
    if (v === 'connector') {
      const { renderCategory } = await import('./category.js');
      openOverlay({ title: NAME + '连接器管理', build: async (body) => { body.style.overflowY = 'auto'; await renderCategory(body); } });
    }
  };
